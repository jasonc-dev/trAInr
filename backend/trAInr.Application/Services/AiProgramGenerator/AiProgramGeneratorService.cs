...
public class AiProgramGeneratorService : IAiProgramGeneratorService
{
  ...
  public async Task<ProgramTemplateResponse> GenerateProgramAsync(GenerateProgamRequest request, CancellationToken cancellationToken = default)
  {
    ...
    var retrievalRequest = new ExerciseRetrievalRequest(
        AvailableEquipment: request.AvailableEquipment ?? Array.Empty<string>(),
    ...
    var candidateExercises = await _exerciseRetrievalService.GetCandidateExercisesAsync(retrievalRequest, cancellationToken);
  ...
  {
    var exerciseJson = JsonSerializer.Serialize(availableExercises, _jsonOptions);
...
Generate the complete program now:";
  ...
  private static void ValidateExerciseIds(AiProgramStructure programStructure, IReadOnlyList<ExercisePromptDto> availableExercises)
  {
    ...
    if (invalidIds.Any())
    {
      throw new InvalidOperationException($"AI selected exercise IDs that don't exist in candidate set: {string.Join(", ", invalidIds)}");
    }
  }
