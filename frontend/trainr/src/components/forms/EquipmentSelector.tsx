 ...
 * Dropdown multi-select checkbox component for equipment preferences
 */
...
export const EquipmentSelector: React.FC<EquipmentSelectorProps> = ({
  ...
  const handleToggle = (equipment: string) => {
    if (selected.includes(equipment)) {
      onChange(selected.filter((item) => item !== equipment));
    } else {
  ...
  const getDisplayText = () => {
    ...
    if (selected.length === 1) {
      const option = EQUIPMENT_OPTIONS.find((opt) => opt.value === selected[0]);
